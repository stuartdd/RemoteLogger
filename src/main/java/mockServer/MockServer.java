...
public class MockServer {
    ...
    public MockServer(int port, ResponseHandler responseHandler, String expectationFile, boolean verbose) {
        server = new Server(port, new ServerConfig(expectationFile, 0, verbose), responseHandler, new MockServerNotifier());
    }
    
    public MockServer(int port, ResponseHandler responseHandler, Expectations expectations, boolean verbose) {
        server = new Server(port, new ServerConfig(expectations, 0, verbose), responseHandler, new MockServerNotifier());
    }
    
    public MockServer(int port, ResponseHandler responseHandler, boolean verbose) {
        server = new Server(port, new ServerConfig(new Expectations(), 0, verbose), responseHandler, new MockServerNotifier());
    }
